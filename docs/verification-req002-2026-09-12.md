...
| Qt 自动化回归 | `passed` | CTest 5/5；13+40+8+6+62＝129 passed，0 failed，0 skipped（包含初始化／清理） |
...
`build/req002/performance/`，不提交。标题密集样本的导出复测与样式拆分记录如下。

标题密集样本导出超过 10% 复核线后，按基线→候选顺序再次测量：P95 为
4.517 → 4.977 ms（+10.2%，约 +0.46 ms），HTML 从 171287 增至 189788 字节（+10.8%）。
正文和代码显式字号、字体及六级标题的不同格式增加了序列化量；保留本单正确排版所需的
格式信息，并如实记录这一成本，没有将其解释为测量噪声或声称所有路径都更快。

下表为独立 `--layers` 的 P95，单位 ms。原生缩放基线没有本单的标题／代码比例修正，
因此“原生滚轮及样式”包含新增行为成本；主题事件计时也包含本轮排队事件和可能发生的布局。

| 样本 | 接入及样式 | 主题事件处理 | 原生滚轮及样式 | 滚轮后剩余强制布局 | 保存配置读取（候选） |
| --- | --- | --- | --- | --- | --- |
| 100k | 2.276 → 2.135 | 40.004 → 4.125 | 0.779 → 2.780 | 15.050 → 12.612 | 0.251 |
| 1m | 20.483 → 17.789 | 287.530 → 25.246 | 9.343 → 31.048 | 137.931 → 107.064 | 0.260 |
| 5m | 20.553 → 18.507 | 1713.121 → 25.754 | 8.981 → 30.504 | 715.530 → 592.911 | 0.913 |
| tables-images | 14.741 → 17.524 | 192.946 → 20.239 | 5.738 → 25.222 | 3.211 → 1.152 | 0.257 |
| headings | 6.841 → 6.192 | 558.604 → 7.204 | 0.722 → 6.746 | 5.836 → 5.108 | 0.264 |

原生滚轮新增比例修正增加约 2～22 ms 的同步处理；图片／表格密集样本接入及样式增加
约 2.8 ms。批量格式操作已避免逐片段布局，仍需遍历当前富文本以保持标题和代码比例；
这是新增排版行为的实际成本。配置读取的 P95 约 0.25～0.91 ms，仅发生在打开时。
...
功能提交 `d89b1949a4fe376cd04cc76317165b24677ff987` 推送后触发 Actions `34701967999`。
Windows Release DLL 编译通过，发布脚本隔离测试通过，Qt 回归为 126 passed／2 failed，
因此没有打包。下载诊断 Artifact 并核对其 SHA256 与 GitHub digest 一致后定位到：

- Windows 检出自动转换宿主 INI 样本的换行符，原字节哈希失败。新增 `.gitattributes`，
  仅将合成 INI 样本设为 `-text`，保持跨平台字节一致。
- Qt 5.15.2 的 `MD_SPAN_CODE` 使用 `setFont(systemFont(FixedFont))`，Windows 返回的
  QFont 可能没有 fixedPitch 标志。Dock 结合导入器的显式字体属性识别代码，保存片段
  身份，并清除可能覆盖所选字体的 FontFamilies／StyleName 属性。新增
  `importedCodeWithUnsetFixedPitchRemainsMonospaced` 在 Linux 上直接重现这组输入。
...
最终视觉检查发现，初版截图用例只改变 Base／Text，导致 AlternateBase 仍是浅色。
补齐测试用例的 AlternateBase、边框与 Link 调色板后重新生成截图；这只修正模拟主题输入，
不修改产品样式或字体代码。真实宿主主题与 DPI 仍需用户验证。
