...
- H1～H6 标题大纲，支持左右布局、显隐、宽度调整和键盘激活
- 最新预览的大纲标题可带动源码一起定位；当前章节标记与键盘选择独立
- 预览内字面搜索，支持大小写、命中计数、临时高亮和前后环绕；待刷新时搜索展示快照
- 代码块语言标识、键盘可访问的完整块复制，以及可记忆的长行显示设置
...
完整渲染达到 750 ms 时暂停后续自动刷新。立即刷新可取得最新内容，成功后显示已更新；若
...
每个可见代码块左上角显示语言，右上角附有小复制图标，点击即可复制该块；成功后图标
短暂变为勾号。按钮支持 Tab 焦点与空格激活，代码内右键菜单也保留完整块复制。
复制保留代码缩进、Tab 和换行，手动模式下明确说明复制当前预览内容；内容不可用或
剪贴板失败时会反馈原因。原生预览未显示的空围栏可通过预览右键菜单复制。

“设置 → 代码块视觉自动换行”默认开启。关闭后保持长行，按需使用预览水平滚动条；
两种模式复制的代码相同。这项偏好独立保存并跨重启恢复，已打开的其他窗口不即时联动。
复制控件、语言标识装饰及提示不进入导出的 HTML。
...
“重置预览缩放”回到本次打开时读取的基准；编辑器与预览的缩放互不影响，也不写入宿主设置。

### 标题大纲与源码导航

插件菜单和预览工具栏的“设置”均可以显示／隐藏标题大纲，并将它放在正文左侧或右侧。
宽度通过鼠标拖动大纲与正文之间的分隔条调整。大纲默认左侧显示且全部展开；这些设置只
保留在当前窗口。可以点击标题或用方向键选择后按 Enter 激活；最新预览会让源码与预览一起定位到同一标题，关闭连续同步开关也不影响
这次主动跳转。目标位于源码折叠内时，会展开必要祖先以显示标题。

滚动预览会更新大纲中的当前章节标记，不抢键盘焦点、不改变选中项或展开状态。
当前章节被折叠隐藏时，祖先显示空心提示。源码有未刷新修改时，大纲仍基于旧预览，激活
只移动预览并提示先刷新；缺少源码定位能力或无法可靠核对标题时也会明确提示。

### 在预览中查找

使用“插件 → Markdown 预览 → 在预览中查找…”、工具栏查找图标，或在预览内按
`Ctrl+F` 打开搜索栏；编辑区的 `Ctrl+F` 仍使用宿主查找。搜索按字面匹配，支持区分
大小写、命中计数与临时高亮，使用 `Enter`／`Shift+Enter` 跳到下一项／上一项并首尾环绕。
修改查询不会立即跳离当前阅读位置，长文档搜索会分批计算并显示进度。

搜索只针对当前显示的预览快照，不触发全文刷新或源码跳转。手动模式有待刷新修改时，
结果属于旧快照；没有有效快照时需先刷新。搜索高亮不进入 HTML 导出。

### 面板布局与菜单

标题栏提供浮动窗口／停靠回主窗口及关闭按钮，文件名过长时省略显示，可查看完整文档
详情。浮动后可用返回按钮恢复到原来的左侧或右侧停靠位置。底部状态栏显示更新状态及
导航反馈，刷新失败时可重试或打开可复制的错误详情。

窄面板会简化同步滚动按钮文字并临时收起大纲，加宽后恢复用户选择；临时收起不会改写
当前窗口的大纲偏好。插件主菜单集中提供刷新模式、显示大纲、大纲位置、查找、同步滚动、
重置预览缩放、导出和关于信息，大纲与面板设置共享同一状态。
